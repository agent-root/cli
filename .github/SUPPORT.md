...
- Versions: `node --version`, the CLI version (`agent-root --version` or check `package.json`)
- OS
...
Protocol-level changes (new record types, DNS conventions, manifest fields) go to the main [agentroot repo](https://github.com/d3-inc/agentroot), not this CLI repo. The CLI consumes the protocol; it doesn't define it.
