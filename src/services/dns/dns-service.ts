...
export type ResolveResult =
  ...
  | { found: true; mode: 'manifest'; manifestUrl: string; raw: string }
  | { found: true; mode: 'skill'; skillUrl: string; raw: string }
  | { found: true; mode: 'inline'; fields: Record<string, string>; raw: string };
...
export async function resolveAgentroot(domain: string): Promise<ResolveResult> {
  ...
  if (!ar1) {
    return { found: false, error: `TXT records found at ${hostname} but none start with v=ar1` };
  }
  ...
  if (manifestUrl) {
    return { found: true, mode: 'manifest', manifestUrl, raw: ar1 };
  } else if (fields.skill) {
    return { found: true, mode: 'skill', skillUrl: fields.skill, raw: ar1 };
  } else if (fields.type) {
    return { found: true, mode: 'inline', fields, raw: ar1 };
  }

  return { found: false, error: `TXT record found but missing manifest=, skill=, and type= fields: ${ar1}` };
}
